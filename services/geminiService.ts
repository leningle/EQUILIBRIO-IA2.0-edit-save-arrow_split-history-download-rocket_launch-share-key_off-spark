...
export const generateTextResponse = async ({ prompt, modelType, history }: GenerateTextParams): Promise<string> => {
  // Default to gemini-3-flash-preview for standard text tasks per guidelines
  let modelName = 'gemini-3-flash-preview';
  let config: any = {};

  switch (modelType) {
    case ModelType.FLASH_LITE:
      // Fixed: Use correct model name 'gemini-flash-lite-latest'
      modelName = 'gemini-flash-lite-latest';
      break;
    case ModelType.PRO:
      // Fixed: Use 'gemini-3-pro-preview' for advanced tasks
    ...
    case ModelType.THINKING:
      ...
      // Important: Do not set maxOutputTokens when using thinkingBudget per documentation
      break;
  }
...
export const generateAvatarImage = async (description: string, referenceImageBase64?: string): Promise<string | null> => {
  try {
    ...
    // Parse response for image data by iterating through all response parts
